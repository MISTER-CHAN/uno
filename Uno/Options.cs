...
namespace Uno {
	public partial class Options : Form {
        ...
        private void LoadGame(string save)
        {
            ...
            mnuMultiply0.Checked = bool.Parse(keys[54]);
            mnuMultiply1.Checked = bool.Parse(keys[55]);
            mnuMultiply10000.Checked = bool.Parse(keys[56]);
        ...
        private void MnuDailyMoney_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Interaction.SaveSetting("UNO", "ACCOUNT", "LAST", DateTime.Today.Ticks + "");
                    int bonus = new Random().Next(9000000, 11000000);
                    Interaction.SaveSetting("UNO", "ACCOUNT", "MONEY", money + bonus + "");
                    money += bonus;
                    mnuMe.Text = "$" + money;
                    MessageBox.Show("+$" + bonus, "簽到", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
